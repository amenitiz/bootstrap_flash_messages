...
module BootstrapFlashMessages
  ALERT_CLASS_MAPPING = Hash.new(:info).merge(notice: :success, success: :success, warning: :warning,
                                              error: :danger).freeze
  ...
  def self.initialize
    ...
    ActionController::Base.include BootstrapFlashMessages::FlashMessages
    @initialized = true
  end
end

require 'bootstrap_flash_messages/railtie' if defined?(Rails::Railtie)
